...
public class DownloadManager {
        ...
        try {
            ...
            if (cursor.moveToFirst()) {
                ...
                if (STATUS_SUCCESSFUL == status) {
                    int index = cursor.getColumnIndexOrThrow(COLUMN_LOCAL_URI);
     ...
     *
     * @param flags any combination of the Request#NETWORK_* bit flags.
     */
    public void setAllowedNetworkTypes(int flags) {
        ContentValues values = new ContentValues();
        values.put(Downloads.COLUMN_ALLOWED_NETWORK_TYPES, flags);
        mResolver.update(mBaseUri, values, null, null);
    }
